function options_default = evalgradient_options(varargin)
% Evaluates gradient of the function f at point x according to Ridders' method:
%
...
%
% OUTPUT:
%    options   An optinoal struct
%       options.init_eps       Initial  finite difference (default: 1)
%       options.ratio          which is used to reduce eps on each step 
%                               (default: 0.8)
%       options.min_steps      Minimum number of steps in eps (default: 5)
%       options.max_steps      Maximum number of steps in eps (default: 100)
%       options.threshold      Terminate if last step worse than preceding
%                              by a factor of threshold
%                               (default: 1.5)
%       options.tolerance      terminal computation if the fact error is
