function [ out_f ] = rosenbrock( xk , objpara )
...
elseif strcmp(rosenModel, 'uncoupled_2D')
   N = length(xk);
    if rem(N ,2) ==0
         ...
         out_f=0;
         for i=1:1:m
             out_f = out_f + a*( xk(2*i-1)^2 - xk(2*i) )^2  + b*( xk(2*i-1) -1 )^2;
         end        
    else
        error('Error:Dimensions are error!');
    end
...
elseif  strcmp(rosenModel,'coupled_2D')
     ...
     out_f=0;
     for i=1:1:N-1
         out_f = out_f + a*( xk(i+1) - xk(i)^2 )^2  + b*( xk(i) -1 )^2;
     end      
else
     error('Error:Parameter is incorrect!');
end
return;     
end
